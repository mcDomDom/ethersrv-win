...
int setitemattr(char *i, unsigned char fattr) {
  if (!SetFileAttributes(i, fattr)) return(-1);
  return(0);
}
...
static long gendirlist(struct sfsdb *root, unsigned char fatflag) {
  ...
  for (;;) {
    ...
    newnode = (struct sdirlist *)calloc(1, sizeof(struct sdirlist));
    if (newnode == NULL) {
      fprintf(stderr, "ERROR: out of mem!");
      break;
    }
...
#endif
	if (ret != 0) continue;	// pagefile.sysなど_accessで-1が返ってくる
...
unsigned long long diskinfo(char *path, unsigned long long *dfree) {
  ...
  if (!GetDiskFreeSpaceEx(path, &avail, &total, &free)) return(0);
  res = total.QuadPart;
  *dfree = avail.QuadPart;
  return(res);
}
...
long writefile(unsigned char *buff, unsigned short fss, unsigned long offset, unsigned short len) {
  ...
  if (len == 0) {
	...
	handle = _open(fname, _O_BINARY | _O_RDWR);
	if (handle == -1) return -1;
    if (_chsize(handle, offset) != 0) fprintf(stderr, "Error: truncate() failed\n");
    return(0);
  }
...
int delfiles(char *pattern) {
  ...
  for (;;) {
    ...
    if (diridx == NULL) break;
    /* skip directories */
    if (diridx->d_type & FILE_ATTRIBUTE_DIRECTORY) continue;
    ...
    if (matchfile2mask(filfcb, dirnamefcb) == 0) {
      ...
      if (_unlink(fname) != 0) fprintf(stderr, "failed to delete '%s'\n", fname);
    }
  }
  closedir(dp);
